function [R_opt, T_opt] = OptimizePose2(X1_o, X2_o, K1, K2, R, T, thrd_iter, max_iter, img_left, img_right,d1,d2,lambda)
% this code optimize the camera pose by minimizing the reprojection errors
% and minimizing the cost of completement
...
while iter_count <= max_iter
    ...
    [R_opt, T_opt] = UpdatePose2(X1_m, X2_m, X1_o, X2_o, R, T, K1, K2, img_left, img_right, d1, d2, lambda);
    % update X1_m, X2_m (minimize the X_o to X_m distancd
    [X1_m, X2_m, err] = UpdateModel(X1_o, X2_o, R_opt, T_opt);
    iter_count = iter_count + 1;
    R = R_opt;
    T = T_opt;
%     R_opt{iter_count}=R_opt_;
%     T_opt{iter_count}=T_opt_;
    errs=[errs;err];
%     figure(90);
%     plot(errs);
...
function [R_opt, T_opt] = UpdatePose2(X1_m, X2_m, X1_o, X2_o, R, T, K1, K2, img1, img2, d1, d2, lambda)
    rt_init = [rodrigues(R); T];
    T_norm = norm(T);
    opt = optimset('Algorithm', 'levenberg-marquardt', 'TolX',1e-20, 'TolFun', 1e-20, 'MaxFunEvals', 1000, 'MaxIter', 1000);
    
    [rt_new,res] = lsqnonlin(@(rt_new) Xm2EpiLineError2(rt_new, X1_m, X2_m, X1_o, X2_o, T_norm, K1, K2, img1, img2, d1, d2, lambda), rt_init, [], [], opt);
    res
...
function obj = Xm2EpiLineError2(rt, X1_m, X2_m, X1_o, X2_o, T_norm, K1, K2, img1, img2, d1, d2, lambda)
    ...
    [X1_r, X2_r, epi_err] = FindEpiCorres2(img1, img2, X1_o, X2_o ,50, R_left, R_right, K1, K2, d1, d2);
    ...
    for i = 1:N
        ...
        obj = [obj;(x2_o * TsR * x1_m')/ norm(e3sTsR * x1_m');(x2_m * TsR * x1_o')/ norm(x2_m * TsRe3sT);lambda*term];
    end
    sum(norm(obj));
end
